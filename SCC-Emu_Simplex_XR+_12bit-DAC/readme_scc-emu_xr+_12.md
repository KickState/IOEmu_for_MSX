# IOEμ: SCC-Emu Simplex XR+ with 12-bit DAC

**An eXtended Simplex build featuring ROM/RAM emulation, SCC+, and 64KB MegaCON support on a single 8-bit PIC.**

## 1. 概要

* SCC-Emu Simplex XR+ with 12-bit DAC (以降、SCC-Emu XR+)は、[SCC-Emu Simplex with 12-bit DAC](/SCC-Emu_Simplex_12bit-DAC/readme_scc-emu_12.md)の派生モデルです。
* Schematic、GerberデータはSCC-Emu Simplexの12-bit DAC版と共通です。Firmwareのみが異なります。
...
使用例は[こちら](https://x.com/kickstate7/status/2042911266566738417)。※Ｘへのリンクです。
...
SCC-Emu XR+ with 12-bit DAC版には、必ず、**12-bit DAC**用のHEXファイルを使用してください。
